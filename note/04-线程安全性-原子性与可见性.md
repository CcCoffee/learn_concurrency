...
- Th**read start rule. A call to Thread.start on a thread happens before every action in the started thread.
...
从JDK1.5之后，**JMM**使用happen-before的概念阐述操作之间的内存可见性。
并保证只要线程A和B之间满足happen-before关系，执行操作B的线程可以看到操作A的结果，
本质在于JMM使用内存屏障禁止了操作重排序，从而实现一种偏序关系。
...
3>4>1>2 (1,0)
```
结果只有 (0,1), (1,0), (1,0) 三种可能

值得一提的是，案例中把所有的变量都设置为volatile，其实若对volatile重排序规则了解的话，
可以知道大可不必这样，因为插入内存屏障会有所开销:po一张volatile排序规则表：
